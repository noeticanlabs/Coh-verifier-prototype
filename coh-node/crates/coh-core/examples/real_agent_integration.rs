...
use coh_core::types::{MetricsWire, MicroReceipt, MicroReceiptWire, Decision};
...
fn main() {
    ...
    for step in 1..=3 {
        ...
        println!("  LLM Claims: Spend={}, Defect={}", llm_payload.spend, llm_payload.defect);
        ...
        match result.decision {
            Decision::Accept => {
                println!("  [SUCCESS] Step is lawful. Committing to state: {}", next_state);
...
fn simulate_llm_step(step: i32) -> MockLLMPayload {
    match step {
        1 => MockLLMPayload { spend: 50, defect: 0 },   // Honest reconciliation
        2 => MockLLMPayload { spend: 2000, defect: 0 }, // HALLUCINATION: spend > v_pre (1000 - 50 = 950)
        _ => MockLLMPayload { spend: 0, defect: 0 },
    }
}
