...
fn test_fixture_oracle_sweep() {
    ...
    for (cmd, file, expected_code, expected_text) in fixtures {
        ...
        assert_eq!(code, expected_code, "Fixture {} failed code check. Stdout: {}", file, stdout);
        assert!(stdout.contains(expected_text), "Fixture {} failed text check. Stdout: {}", file, stdout);
