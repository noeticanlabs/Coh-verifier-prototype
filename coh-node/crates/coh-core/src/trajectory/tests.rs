...
mod tests {
    ...
    use crate::trajectory::domain::{
        ...
        OpsState, OpsStatus, OpsAction, AgentState, AgentStatus
    };
    ...
    fn test_hash_grounding() {
        let s1 = DomainState::Financial(FinancialState { balance: 1000, initial_balance: 1000, status: FinancialStatus::Idle, current_invoice_amount: 0 });
        let s2 = DomainState::Financial(FinancialState { balance: 1000, initial_balance: 1000, status: FinancialStatus::Invoiced, current_invoice_amount: 1000 });
        
        let h1 = crate::hash::sha256(&serde_json::to_vec(&s1).unwrap());
        let h2 = crate::hash::sha256(&serde_json::to_vec(&s2).unwrap());
        
        assert_ne!(h1, h2, "State hashes must be distinct for distinct semantic states");
    }
    ...
    fn test_bottleneck_vs_shortcut() {
        ...
        for _ in 0..3 {
            traj_a.steps.push(VerifiedStep::new(
                DomainState::Financial(FinancialState { balance: 1000, initial_balance: 1000, status: FinancialStatus::Idle, current_invoice_amount: 0 }),
                Action::Financial(FinancialAction::VerifyVendor),
                DomainState::Financial(FinancialState { balance: 1000, initial_balance: 1000, status: FinancialStatus::Idle, current_invoice_amount: 0 }),
                Hash32::default(), Hash32::default(), AcceptWitness
            ));
        ...
        let mut traj_b = AdmissibleTrajectory::new();
        traj_b.steps.push(VerifiedStep::new(
            DomainState::Financial(FinancialState { balance: 400, initial_balance: 1000, status: FinancialStatus::Idle, current_invoice_amount: 0 }),
            Action::Financial(FinancialAction::VerifyVendor),
            DomainState::Financial(FinancialState { balance: 400, initial_balance: 1000, status: FinancialStatus::Idle, current_invoice_amount: 0 }),
            Hash32::default(), Hash32::default(), AcceptWitness
        ));
    ...
    fn test_alignment_index_setback() {
        let s_acting = DomainState::Agent(AgentState { complexity_index: 10, complexity_budget: 100, authority_level: 1, status: AgentStatus::Acting });
        let s_review = DomainState::Agent(AgentState { complexity_index: 10, complexity_budget: 100, authority_level: 2, status: AgentStatus::PolicyReview });
        
        assert!(s_acting.alignment_index() > s_review.alignment_index(), "PolicyReview must be a setback");
    }
}
